...
async function init () {
  ...
  */
  ...
  const defaultNoteId = addTemplate('Default Note', new Note(null, '', 'New Note', ['default']))
  addTemplate('Meeting', new Note(null, MEETING_NOTES, 'New Meeting Note', ['meeting']))
  addTemplate('Freeform MD Note', new Note(null, FREEFORM_MD, 'New Freeform MD Note', ['freeform']))
  addTemplate('Design', new Note(null, DESIGN_NOTES, 'New Design Note', ['design']))
  addTemplate('Github', new Note(null, GITHUB_NOTES, 'New Github Note', ['github']))
  addTemplate('Code', new Note(null, CODE_AND_BUG_SNIPPETS, 'New Code Note', ['code']))
  // populate select-note div
  const selectNote = document.querySelector('.select-note')
  const SELECTED_COL = 'darkblue'
  const OTHER_COL = 'darkcyan'
  ...
  */
  ...
  document.querySelector('#choose-note').addEventListener('click', () => {
    ...
    // don't exit the popup if we failed
    if (man.addNote(noteToAdd) === null) return
